...
const EditableSlide = React.memo(React.forwardRef<HTMLDivElement, {
  ...
  return (
    <div
      ref={ref}
      className={`slide-container bg-white dark:bg-slate-900 shadow-xl border border-slate-100 dark:border-slate-800 transition-all box-border w-full ${style.template === 'blueprint' ? 'bg-indigo-900 text-white' :
        style.template === 'minimalist' ? 'shadow-none border-2 dark:border-slate-700' : ''
        }`}
      style={{
      ...
      <div className="p-2 md:p-8 relative flex flex-col justify-start h-full overflow-hidden">
        <div className="flex-1 overflow-y-auto pr-4 custom-scrollbar">
          {activeTextPart && activeTextPart.slideId === slide.id && (
            <div
              ref={toolbarRef}
              className="mb-4 bg-white/95 dark:bg-slate-900/95 backdrop-blur-xl shadow-lg rounded-2xl p-2 flex flex-wrap justify-center items-center gap-y-2 gap-x-3 border border-slate-200 dark:border-slate-800 no-print animate-in fade-in zoom-in duration-200 ring-1 ring-slate-900/5 mx-auto w-full md:w-auto md:max-w-none md:flex-nowrap"
            >
              {/* Linha 1 (Mobile) / Grupo Esquerda (Desktop) */}
              <div className="flex items-center gap-1 shrink-0">
                <button
                  onMouseDown={(e) => { e.preventDefault(); document.execCommand('bold', false, ''); }}
                  className="w-8 h-8 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800 flex items-center justify-center transition-colors font-bold text-slate-700 dark:text-slate-300"
                  title="Negrito"
                >
                ...
                <button
                  ...
                  className="w-8 h-8 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800 flex items-center justify-center transition-colors italic text-slate-700 dark:text-slate-300"
                  title="Itálico"
                >
                ...
                <button
                  ...
                  className="w-8 h-8 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800 flex items-center justify-center transition-colors underline text-slate-700 dark:text-slate-300"
                  title="Sublinhado"
                >
                ...
                <div className="flex items-center gap-1 bg-slate-100 dark:bg-slate-800 rounded-lg px-2 py-1.5" title="Tamanho da Fonte">
                  <i className="fa-solid fa-text-height text-slate-400 dark:text-slate-500 text-[10px]"></i>
                  <input
                    type="number"
                    ...
                    className="w-14 bg-transparent text-xs font-bold outline-none text-center px-1 appearance-none text-slate-700 dark:text-slate-300"
                  />
                </div>
                <div className="flex items-center gap-1 bg-slate-100 dark:bg-slate-800 rounded-lg px-2 py-1.5" title="Altura da Linha">
                  <i className="fa-solid fa-arrows-up-down text-slate-400 dark:text-slate-500 text-[10px]"></i>
                  <input
                    type="number"
                    step="0.1"
              ...
              <div className="flex items-center gap-1 shrink-0">
                {['left', 'center', 'right', 'justify'].map((align) => (
                  <button
                    ...
                    className={`w-8 h-8 rounded-lg flex items-center justify-center transition-colors ${style.bodyStyles.textAlign === align ? 'bg-indigo-100 dark:bg-indigo-900/40 text-indigo-600 dark:text-indigo-400' : 'hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-400 dark:text-slate-500'}`}
                    title={`Alinhar ${align}`}
                  >
          ...
          <div className="flex justify-between items-center no-print mb-8" data-html2canvas-ignore>
            <span className="text-xs font-black opacity-30 dark:opacity-50 uppercase tracking-widest dark:text-slate-400">SLIDE {idx + 1}</span>
            <div className="flex gap-2">
              <div className="flex bg-slate-100 dark:bg-slate-800 rounded-full px-1 py-1 mr-2 no-print">
                <button
                  onClick={() => onMove?.(idx, 'up')}
                  disabled={idx === 0}
                  className={`w-6 h-6 rounded-full flex items-center justify-center transition-all ${idx === 0 ? 'opacity-20 cursor-not-allowed' : 'hover:bg-indigo-600 hover:text-white text-slate-500 dark:text-slate-400 dark:hover:text-white'}`}
                ...
                <button
                  ...
                  className={`w-6 h-6 rounded-full flex items-center justify-center transition-all ${totalSlides !== undefined && idx === totalSlides - 1 ? 'opacity-20 cursor-not-allowed' : 'hover:bg-indigo-600 hover:text-white text-slate-500 dark:text-slate-400 dark:hover:text-white'}`}
              ...
              {onPreview && (
                <button
                  onClick={() => onPreview?.('')}
                  className="text-[10px] bg-indigo-50 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400 px-4 py-1.5 rounded-full font-bold hover:bg-indigo-600 hover:text-white transition-all shadow-sm border border-indigo-100 dark:border-indigo-800"
                >
                  <i className="fa-solid fa-eye mr-1"></i> ORIGINAL
                </button>
              )}
              <button onClick={() => onExport(slide.id)} className="text-[10px] bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 px-4 py-1.5 rounded-full font-bold hover:bg-indigo-600 hover:text-white transition-all shadow-sm">
          ...
          <div
            className={`group relative outline-none transition-all hover:bg-indigo-50/5 dark:hover:bg-white/2 rounded-xl p-3 cursor-text space-y-6 text-slate-800 dark:text-white ${activeTextPart?.slideId === slide.id ? 'ring-2 ring-indigo-500/50 bg-indigo-50/10 dark:bg-white/5 shadow-inner' : ''}`}
            style={{
          ...
          >
            {slide.content.map((point, pIdx) => (
              <div key={pIdx} className="flex items-start gap-5">
                <span className="opacity-20 dark:opacity-40 mt-2.5 text-xs text-indigo-500">◆</span>
                <div
                  contentEditable
                  ...
                  style={{ color: style.template === 'blueprint' ? '#e0e7ff' : (isDarkMode ? '#ffffff' : style.bodyStyles.color) }}
...
const App: React.FC = () => {
  ...
  const [isDarkMode, setIsDarkMode] = useState(() => localStorage.getItem('lousa_theme') === 'dark');

  useEffect(() => {
    if (isDarkMode) {
      document.documentElement.classList.add('dark');
      localStorage.setItem('lousa_theme', 'dark');
    } else {
      document.documentElement.classList.remove('dark');
      localStorage.setItem('lousa_theme', 'light');
    }
  }, [isDarkMode]);
  ...
  return (
    <div className={`h-screen flex flex-col bg-slate-50 dark:bg-slate-950 overflow-x-hidden max-w-[100vw] transition-colors duration-300`}>

      <header className="bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 sticky top-0 z-50 no-print transition-colors">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center gap-3 px-4 py-3">
          <div className="flex items-center gap-3 cursor-pointer group" onClick={() => setActiveProject(null)}>
            ...
            <h1 className="text-xl font-black tracking-tighter text-slate-900 dark:text-white">LOUSA <span className="text-indigo-600">IQ</span></h1>
          </div>
          ...
          <div className="w-full md:flex-1 md:max-w-xl flex items-center gap-3">
            <div className="relative group flex-1">
              ...
              <input
                ...
                className="w-full bg-slate-100 dark:bg-slate-800 border-none rounded-full py-2 pl-10 pr-4 text-sm focus:ring-2 focus:ring-indigo-500 outline-none transition-all dark:text-white dark:placeholder:text-slate-500"
              />
            </div>

            <button
              onClick={() => setIsDarkMode(!isDarkMode)}
              className="w-10 h-10 rounded-full flex items-center justify-center bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-indigo-400 hover:rotate-12 transition-all shadow-sm"
      ...
      <main className="flex-1 max-w-7xl mx-auto w-full px-4 md:px-6 py-4 md:py-6 overflow-y-auto">
        {(isLoading || pdfGenerating) && (
          <div className="fixed inset-0 bg-white/80 dark:bg-slate-950/80 backdrop-blur-md z-[100] flex items-center justify-center flex-col no-print">
            <div className="w-20 h-20 border-4 border-indigo-600 border-t-transparent rounded-full animate-spin mb-6"></div>
            <p className="text-xl font-black text-slate-800 dark:text-white tracking-widest uppercase">{pdfGenerating ? "GERANDO PDF FINAL..." : "DIGITALIZANDO QUADRO..."}</p>
          </div>
        )}
        ...
        {!activeProject ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 no-print">
            {filteredProjects.map(p => (
              <div key={p.id} onClick={() => setActiveProject(p)} className="bg-white dark:bg-slate-900 rounded-2xl shadow-sm hover:shadow-xl transition-all border dark:border-slate-800 overflow-hidden cursor-pointer group relative">
                ...
                <button
                  ...
                  className="absolute top-3 right-3 bg-white/90 dark:bg-slate-800/90 backdrop-blur shadow-md w-8 h-8 rounded-full flex items-center justify-center text-red-500 opacity-0 group-hover:opacity-100 transition-opacity hover:bg-red-500 hover:text-white"
                  title="Excluir Aula"
                >
                ...
                <div className="p-5">
                  ...
                  <h3 className="font-bold text-slate-800 dark:text-slate-100 mb-3">{p.name}</h3>
          ...
          <div className="flex flex-col-reverse lg:grid lg:grid-cols-12 gap-8 w-full overflow-visible">
            <div className="lg:col-span-12 flex flex-col md:flex-row justify-between items-start md:items-end gap-6 border-b dark:border-slate-800 pb-8 no-print">
              ...
              <div className="flex flex-col md:flex-row gap-2 w-full md:w-auto">
                ...
                <button onClick={exportAllAsPDF} className="bg-slate-900 dark:bg-indigo-600 text-white px-5 py-3 md:py-2.5 rounded-xl font-bold hover:bg-slate-800 dark:hover:bg-indigo-700 transition-all shadow-lg w-full md:w-auto flex justify-center items-center">
                ...
                <button
                  ...
                  className="bg-slate-900 dark:bg-slate-800 text-white px-5 py-3 md:py-2.5 rounded-xl font-bold hover:bg-slate-800 dark:hover:bg-slate-700 transition-all w-full md:w-auto flex justify-center items-center shadow-lg"
                >
                ...
                <button
                  ...
                  className="bg-indigo-50 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400 border border-indigo-200 dark:border-indigo-800 px-5 py-3 md:py-2.5 rounded-xl font-bold hover:bg-indigo-600 hover:text-white transition-all w-full md:w-auto flex justify-center items-center shadow-sm"
                >
            ...
            <div className="lg:col-span-3 space-y-6 no-print w-full max-w-full min-w-0">
              {showAppearance && (
                <div className="bg-white dark:bg-slate-900 p-6 rounded-3xl border dark:border-slate-800 shadow-xl space-y-6 animate-in slide-in-from-left-4 duration-300">
                  <h4 className="font-black text-slate-900 dark:text-white uppercase tracking-widest text-xs border-b dark:border-slate-800 pb-2">Layout Base</h4>
                  <div>
                    <label className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase mb-2 block">Aspect Ratio</label>
                    <div className="grid grid-cols-2 gap-2">
                      {['16:9', '9:16', '1:1', '4:3'].map(r => (
                        <button key={r} onClick={() => updateStyle({ aspectRatio: r as any })} className={`p-2 rounded-lg border dark:border-slate-700 text-[10px] font-bold ${activeProject.style.aspectRatio === r ? 'border-indigo-600 bg-indigo-50 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400' : 'dark:text-slate-400'}`}>{r}</button>
                      ))}
                    </div>
                  </div>
                  <div>
                    <label className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase mb-2 block">Template</label>
                    <div className="grid grid-cols-2 gap-2">
                      {['modern', 'minimalist', 'blueprint', 'classic'].map(t => (
                        <button key={t} onClick={() => updateStyle({ template: t as any })} className={`p-2 rounded-lg border dark:border-slate-700 text-[10px] font-bold uppercase ${activeProject.style.template === t ? 'border-indigo-600 bg-indigo-50 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400' : 'dark:text-slate-400'}`}>{t}</button>
                      ))}
                    </div>
                  </div>
                </div>
        ...
        <footer className="mt-20 no-print text-center opacity-30 text-xs font-bold uppercase tracking-widest w-full py-8 dark:text-slate-500 transition-colors">
      ...
      {
        showUploadModal && (
          <div className="fixed inset-0 bg-slate-900/60 dark:bg-slate-950/80 backdrop-blur-sm z-[100] flex items-center justify-center p-4 no-print">
            <div className="bg-white dark:bg-slate-900 rounded-3xl p-6 w-[95%] md:max-w-2xl md:w-full shadow-2xl scale-in-center max-h-[90vh] overflow-y-auto border dark:border-slate-800">
              ...
              <div className="flex gap-2 overflow-x-auto pb-4 mb-6 border-b dark:border-slate-800">
                {tempImages.map((img, i) => (
                  <div key={i} className="relative min-w-[120px] h-20 rounded-lg overflow-hidden border dark:border-slate-800">
              ...
              <div className="grid grid-cols-2 gap-4">
                <input
                  ...
                  className="w-full p-3 rounded-xl border dark:border-slate-800 dark:bg-slate-800 dark:text-white focus:ring-2 focus:ring-indigo-500 outline-none transition-all dark:placeholder:text-slate-500"
              ...
              <div className="flex gap-3 mt-4">
                <button onClick={() => setShowUploadModal(false)} className="flex-1 py-3 font-bold text-slate-500 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800 rounded-xl transition-colors">Cancelar</button>
      ...
      {previewImage && (
        <div
          className="fixed inset-0 bg-slate-900/95 dark:bg-black/95 backdrop-blur-xl z-[200] p-4 md:p-12 flex flex-col items-center justify-center animate-in fade-in duration-300"
